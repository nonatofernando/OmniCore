$(document).ready(function () {
    ...
    $(document).on("click", ".btn-editar-cliente", function () {
        ...
        abrir_modal_edicao(id_cliente);
    });

    $(document).on(
        "click",
        ".close-modal-btn, .close-modal-trigger",
        function () {
    ...
    $(document).on("click", ".fixed.inset-0", function (e) {
        if (
            $(e.target).is(".fixed.inset-0") ||
            $(e.target).hasClass("backdrop-blur-sm")
        ) {
    ...
    $.ajax({
        ...
        success: (res) => {
            ...
            if (clientes.length === 0) {
                grid.append(
                    '<p class="col-span-full text-center text-gray-500 py-10 font-medium">Nenhum cliente encontrado.</p>',
                );
                return;
            }

            clientes.forEach((cliente) => {
                const inicial = cliente.nome
...
function abrir_modal_edicao(id) {
    ...
    $("body").addClass("overflow-hidden");
    $body.html(
        '<div class="text-center py-10 text-cyan-500 animate-pulse font-bold">Carregando dados...</div>',
    );
...
function salvar_cliente() {
    ...
    if (!dados.nome)
        return mostrarFeedback(
            "erro",
            "Campo Obrigatório",
            "O nome do cliente é necessário.",
        );
...
function mostrarFeedback(tipo, titulo, msg) {
    ...
    btn.removeClass(
        "bg-emerald-500 hover:bg-emerald-600 bg-red-500 hover:bg-red-600",
    );

    if (tipo === "sucesso") {
        icon.html("✓").addClass("bg-emerald-500/20 text-emerald-500");
        btn.text("Ótimo").addClass(
            "bg-emerald-500 text-white hover:bg-emerald-600",
        );
    } else {
        icon.html("!").addClass("bg-red-500/20 text-red-500");
        btn.text("Tentar novamente").addClass(
